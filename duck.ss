      ...
      (define lift
        (lambda (x)
          (match x
            [,x (guard (atom? x)) x]
            [(let ([,fn* (lambda (,u* ...) ,[e*])] ...) ,[e] )
              (log-debug "let exp=~a" x)
              (let ((free (find-free `(let ([,fn* (lambda (,u* ...) ,e* )] ...) ,e ) ) ))
                (log-debug "find-free=~a fn=~a u*=~a" free fn* u*)
                (set-car! u*  (append (car u*) free) )
                (log-debug "get==>~a" `([,fn* (lambda ( ,u*  ) ,e*)] ... )  )
                (log-debug "=============>")
                (if (null? free)
                  (add-top* `([,fn* (lambda (,u* ...) ,e* )] ... ))
                  (add-top* (add-params `([,fn* (lambda ( ,u* ... ) ,e* ) ] ... ) (car fn*) free))
                )
